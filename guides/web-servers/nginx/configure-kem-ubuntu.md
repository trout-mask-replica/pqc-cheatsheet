# Configure PQC key exchange for nginx on Ubuntu
## TL;DR
Add the following to your SSL-config-file:
...
Open the file that holds the ssl-config for your site with a text editor and add the following lines:
    ...
    ssl_protocols TLSv1.3;
    ssl_conf_command Curves X25519MLKEM768:SecP256r1MLKEM768:SecP384r1MLKEM1024:MLKEM512:MLKEM768:MLKEM1024;
...
If the client doesn't have support for PQC then you need to offer fallback to classical curves/groups. For instance you could add x25519 to your config.
    ...
    ssl_conf_command Curves X25519MLKEM768:X25519;   

## Option 2 - using an Ubuntu version without OpenSSL 3.5 as default
Haven't tried this yet. Worth noting is that it can require building nginx from source and linking it to a non default OpenSSL provider.
